		    ...
		    double alpha, double beta, double gamma)
{
	...
	hkl_parameter_set_value(self->a, a, NULL);
	hkl_parameter_set_value(self->b, b, NULL);
	hkl_parameter_set_value(self->c, c, NULL);
	hkl_parameter_set_value(self->alpha, alpha, NULL);
	hkl_parameter_set_value(self->beta, beta, NULL);
	hkl_parameter_set_value(self->gamma, gamma, NULL);
...
int hkl_lattice_get_B(const HklLattice *self, HklMatrix *B)
{
	...
	c_alpha = cos(hkl_parameter_get_value(self->alpha));
	c_beta = cos(hkl_parameter_get_value(self->beta));
	c_gamma = cos(hkl_parameter_get_value(self->gamma));
	...
	s_alpha = sin(hkl_parameter_get_value(self->alpha));
	s_beta  = sin(hkl_parameter_get_value(self->beta));
	s_gamma = sin(hkl_parameter_get_value(self->gamma));

	b11 = HKL_TAU / (hkl_parameter_get_value(self->b) * s_alpha);
	b22 = HKL_TAU / hkl_parameter_get_value(self->c);
	tmp = b22 / s_alpha;

	B->data[0][0] = HKL_TAU * s_alpha / (hkl_parameter_get_value(self->a) * D);
...
int hkl_lattice_reciprocal(const HklLattice *self, HklLattice *reciprocal)
{
	...
	hkl_lattice_set(reciprocal,
			HKL_TAU * s_alpha / (hkl_parameter_get_value(self->a) * D),
			HKL_TAU * s_beta  / (hkl_parameter_get_value(self->b) * D),
			HKL_TAU * s_gamma / (hkl_parameter_get_value(self->c) * D),
	...
	case 1:
		if (self->alpha->fit) {
			...
			hkl_vector_rotated_around_vector(&b, &axe,
							 hkl_parameter_get_value(self->gamma));

			/* randomize c */
			hkl_vector_randomize_vector(&axe, &a);
			hkl_vector_rotated_around_vector(&c, &axe,
							 hkl_parameter_get_value(self->beta));

			/* compute the alpha angle. */
			hkl_parameter_set_value(self->alpha, hkl_vector_angle(&b, &c), NULL);
			...
			hkl_vector_rotated_around_vector(&c, &axe,
							 hkl_parameter_get_value(self->alpha));

			/* compute beta */
			hkl_parameter_set_value(self->beta, hkl_vector_angle(&a, &c), NULL);
		} else {
			/* gamma */
			a = c = vector_x;
			...
			/* compute gamma */
			hkl_parameter_set_value(self->gamma, hkl_vector_angle(&a, &b), NULL);
		}
		break;
	case 2:
		if (self->alpha->fit) {
