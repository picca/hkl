...
HklSample *hkl_sample_new_copy(const HklSample *self)
{
	...
	HKL_LIST_ALLOC(dup->reflections, len);
	for(i=0; i<len; ++i)
		dup->reflections[i] = hkl_sample_reflection_new_copy(self->reflections[i]);
...
HklSampleList *hkl_sample_list_new_copy(const HklSampleList *self)
{
	...
	HKL_LIST_ALLOC(dup->samples, HKL_LIST_LEN(self->samples));
 ...
 *
 * todo: this method should return an int and not a size_t, as -1 is
 * returned if the sample doesn not exist in the HklSampleList BUG
 * todo: test method
 **/
size_t hkl_sample_list_get_idx_from_name(HklSampleList *self, const char *name)
{
	size_t idx;
