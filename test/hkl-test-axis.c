...
HKL_TEST_SUITE_FUNC( new_copy )
{
	...
	HKL_ASSERT_EQUAL(-M_PI, axis->config.range.min);
	HKL_ASSERT_EQUAL(M_PI, axis->config.range.max);
	HKL_ASSERT_EQUAL(0., axis->config.current);
	HKL_ASSERT_EQUAL(0., axis->config.consign);
	HKL_ASSERT_EQUAL(1, axis->config.dirty);

	copy = hkl_axis_new_copy(axis);

	HKL_ASSERT_EQUAL(0, strcmp("omega", copy->name));
	HKL_ASSERT_EQUAL(-M_PI, copy->config.range.min);
	HKL_ASSERT_EQUAL(M_PI, copy->config.range.max);
	HKL_ASSERT_EQUAL(0., copy->config.current);
	HKL_ASSERT_EQUAL(0., copy->config.consign);
...
HKL_TEST_SUITE_FUNC( get_set_config )
{
	...
	HKL_ASSERT_EQUAL(-M_PI, config.range.min);
	HKL_ASSERT_EQUAL(M_PI, config.range.max);
	HKL_ASSERT_EQUAL(0., config.current);
	HKL_ASSERT_EQUAL(0., config.consign);
	...
	HKL_ASSERT_EQUAL(-1, axis->config.range.min);
	HKL_ASSERT_EQUAL(1, axis->config.range.max);
	HKL_ASSERT_EQUAL(0.5, axis->config.current);
	HKL_ASSERT_EQUAL(0.5, axis->config.consign);
	HKL_ASSERT_EQUAL(1, axis->config.dirty);
...
HKL_TEST_SUITE_FUNC( get_quaternions )
{
	...
	HKL_ASSERT_DOUBLES_EQUAL(1.,qc.data[0], HKL_EPSILON);
	HKL_ASSERT_DOUBLES_EQUAL(0,qc.data[1], HKL_EPSILON);
	HKL_ASSERT_DOUBLES_EQUAL(0,qc.data[2], HKL_EPSILON);
	HKL_ASSERT_DOUBLES_EQUAL(0,qc.data[3], HKL_EPSILON);
	...
	HKL_ASSERT_DOUBLES_EQUAL(0,q.data[2], HKL_EPSILON);
	HKL_ASSERT_DOUBLES_EQUAL(0,q.data[3], HKL_EPSILON);
	HKL_ASSERT_DOUBLES_EQUAL(1./sqrt(2.),qc.data[0], HKL_EPSILON);
	HKL_ASSERT_DOUBLES_EQUAL(1./sqrt(2.),qc.data[1], HKL_EPSILON);
	HKL_ASSERT_DOUBLES_EQUAL(0,qc.data[2], HKL_EPSILON);
	HKL_ASSERT_DOUBLES_EQUAL(0,qc.data[3], HKL_EPSILON);
