...
std::ostream &
operator <<(std::ostream & flux, hkl::smatrix const & m)
{
  flux << std::endl;
  flux << std::showpoint << std::showpos;
  flux << m.get(0,0) << '\t' << m.get(0,1) << '\t' << m.get(0,2) << std::endl;
  flux << m.get(1,0) << '\t' << m.get(1,1) << '\t' << m.get(1,2) << std::endl;
  flux << m.get(2,0) << '\t' << m.get(2,1) << '\t' << m.get(2,2) << std::endl;
