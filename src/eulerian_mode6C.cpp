...
// $Source: /usr/local/CVS/Libraries/HKL/src/Attic/eulerian_mode6C.cpp,v $
  ...
  throw (HKLException)
{
  ...
  if (fabs(det) < mathematicalConstants::getEpsilon1())
    det = 0.;
  if (det < -mathematicalConstants::getEpsilon1())
    throw HKLException(
      "Unobtainable reflection",
  ...
  if (Kf.get_Y() < -mathematicalConstants::getEpsilon1())
  ...
  nu = acos(cc);
  if (Kf.get_Z() < -mathematicalConstants::getEpsilon1())
    nu = - nu;
