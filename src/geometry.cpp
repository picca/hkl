...
namespace hkl
{
	 ...
	 */
	hkl_axis * Geometry::get_axe(const std::string & name) throw(hkl::HKLException)
	{
		size_t idx;

		idx = hkl_axis_get_idx_by_name(_holders->axes, name.c_str());
		return &_holders->axes->axes[idx];
	}
	 ...
	 * @throw hkl::HKLException if the hkl::Axe does not exist.
	 */
	hkl_axis * Geometry::get_axe(const std::string & name) const throw(hkl::HKLException)
	{
		size_t idx;
	...
	void Geometry::get_sample_quaternion(hkl_quaternion * q) const
	{
		...
		hkl_holder_apply_quaternion(_holders->holders, q);
	}
	...
	void Geometry::get_sample_quaternion_consign(hkl_quaternion * q) const
	{
		...
		hkl_holder_apply_quaternion_consign(_holders->holders, q);
	}
	...
	void Geometry::get_Q(hkl_svector * Q) const
	{
		...
		hkl_holder_apply_quaternion(_holders->holders, &qr);
	...
	void Geometry::get_Q_consign(hkl_svector * Q) const
	{
		...
		// compute the qr quaternion of the detector holder.
		hkl_holder_apply_quaternion_consign(_holders->holders, &qr);
	...
	void Geometry::get_kf(hkl_svector * kf) const
	{
		...
		// compute the qr quaternion of the detector holder.
		hkl_holder_apply_quaternion(&_holders->holders[1], &qr);
	...
	void Geometry::get_kf_consign(hkl_svector * kf) const
	{
		...
		// compute the qr quaternion of the detector holder.
		hkl_holder_apply_quaternion_consign(&_holders->holders[1], &qr);
