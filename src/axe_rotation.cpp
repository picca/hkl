 ...
 */
    Rotation::Rotation (const std::string & name,
			...
			const hkl::Value & min, const hkl::Value & current,
			const hkl::Value & max,
			const hkl::svector & axe) throw (hkl::
							 HKLException):hkl::
      Axe (name, description, min, current, max)
    {
    ...
    Rotation::Rotation (const hkl::axe::Rotation & source):hkl::Axe (source),
      _axe (source._axe), _quaternion (source._quaternion)
    {
