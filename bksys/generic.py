...
class genobj:
  ...
  def shlib(self):
    ret = self.env.SharedLibrary(self.target, self.source)
  ...
  def execute(self):
    ...
    for lib in self.uselibs:
      LIB = lib.upper()
      if self.env.has_key(LIB+'_CACHED'):
        if self.env.has_key(LIB+'_CPPPATH'): self.env.AppendUnique(CPPPATH = self.env[LIB+'_CPPPATH'])
        if self.env.has_key(LIB+'_LIBPATH'): self.env.AppendUnique(LIBPATH = self.env[LIB+'_LIBPATH'])
        if self.env.has_key(LIB+'_LIBS'): self.env.AppendUnique(LIBS = self.env[LIB+'_LIBS'])
        if self.env.has_key(LIB+'_CCFLAGS'): self.env.AppendUnique(CCFLAGS = self.env[LIB+'_CCFLAGS'])
